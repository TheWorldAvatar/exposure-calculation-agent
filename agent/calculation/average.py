...
def average(calculation_input: CalculationInput):
    ...
    with open("agent/calculation/resources/average.sql", "r") as f:
        average_sql = f.read()
    ...
    with postgis_client.connect() as conn:
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            ...
            average_sql = average_sql.format(TEMP_TABLE=temp_table)
            ...
            for iri, point in tqdm(iri_to_point_dict.items(), mininterval=60, ncols=80, file=sys.stdout):
                ...
                cur.execute(average_sql, replacements)
