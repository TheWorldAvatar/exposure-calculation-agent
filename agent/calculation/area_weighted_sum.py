...
def area_weighted_sum(calculation_input: CalculationInput):
    ...
    with open("agent/calculation/resources/temp_table_area_weighted.sql", "r") as f:
        temp_table_sql = f.read()

    with open("agent/calculation/resources/area_weighted_sum.sql", "r") as f:
        area_weighted_sum_sql = f.read()
    ...
    with postgis_client.connect() as conn:
        with conn.cursor() as cur:
            ...
            for iri, point in tqdm(iri_to_point_dict.items(), mininterval=60, ncols=80, file=sys.stdout):
                ...
                cur.execute(area_weighted_sum_sql, replacements)
                if cur.description:
