...
# weights are stored on a separate column, at the moment weight column is hardcoded for ggva


def weighted_average(calculation_input: CalculationInput):
    ...
    with open("agent/calculation/resources/temp_table_weighted.sql", "r") as f:
        temp_table_sql = f.read()

    with open("agent/calculation/resources/weighted_average.sql", "r") as f:
        weighted_average_sql = f.read()
    ...
    # hardcoded for ggva
    if 'h1' in calculation_input.calculation_metadata.column_name:
        weight_column = 'h1_weight'
    elif 'h2' in calculation_input.calculation_metadata.column_name:
        weight_column = 'h2_weight'
    elif 'h3' in calculation_input.calculation_metadata.column_name:
        weight_column = 'h3_weight'
    elif 'h4' in calculation_input.calculation_metadata.column_name:
        weight_column = 'h4_weight'
    elif 'h5' in calculation_input.calculation_metadata.column_name:
        weight_column = 'h5_weight'
    else:
    ...
    with postgis_client.connect() as conn:
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            ...
            weighted_average_sql = weighted_average_sql.format(
                TEMP_TABLE=temp_table)
            ...
            for iri, point in tqdm(iri_to_point_dict.items(), mininterval=60, ncols=80, file=sys.stdout):
                ...
                cur.execute(weighted_average_sql, replacements)

                if cur.description:
                    query_result = cur.fetchall()
                    if len(query_result) == 1:
                        subject_to_result_dict[iri] = ExposureValue(
                            value=query_result[0]['exposure_result'])
    ...
    complete_message = 'Completed calculation for average'
