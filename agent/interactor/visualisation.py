...
def create_layer():
    ...
    calculation = _get_calculation(rdf_type, distance, dataset_filters[0])
...
def _get_calculation(rdf_type: str, distance: float, dataset_filter: dict) -> CalculationMetadata:
    from agent.utils.kg_client import kg_client
    ...
    if len(dataset_filter) > 0:  # this one counts the number of key-value pairs
        ...
        query = query_template.format(rdf_type=rdf_type, has_distance=constants.HAS_DISTANCE,
                                      dataset_filter_clauses="\n".join(
                                          dataset_filter_where_clauses), blazegraph_url=BLAZEGRAPH_URL, distance=str(distance))
    ...
    if len(dataset_filter) == 0:
        ...
        query = query_template.format(rdf_type=rdf_type, has_distance=constants.HAS_DISTANCE,
                                      dataset_filter_clauses=filter_not_exists, blazegraph_url=BLAZEGRAPH_URL, distance=str(distance))
...
def _update_data_json(host, layer_group_name, exposure, calculation, set_id, geoserver_layer_name, geoserver_workspace, num_bin):
    ...
    # percentile_range = {"Low (0-33)": (0, 33),
    #                     "Medium (33-67)": (33, 67), "High (67-100)": (67, 100)}
    # percentile_colour = {
    #     "Low (0-33)": "blue", "Medium (33-67)": "green", "High (67-100)": "red"}
    # percentile_list = ["Low (0-33)", "Medium (33-67)", "High (67-100)"]
